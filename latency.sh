...
######################################################################
# test bytedance host base command
# args:
#  -n test round number(default 100)
#  -s without ssl(default with ssl)
#  -h customized host
#  -u customized url(without api suffix)
# Demo:
#    $ bash latency.sh
#
# - test all bytedance default host with 1000 times request
#    $ bash latency.sh -n 1000
#
# - test customized host
#    $ bash latency.sh -h rec-b.volcengineapi.com -n 10
#
# - test customized url
#    $ bash latency.sh -u http://www.baidu.com -n 10
#######################################################################

hosts=(
# default region host
  CN  rec.volcengineapi.com
  CN  rec-b.volcengineapi.com
  SG  rec-ap-singapore-1.byteplusapi.com
  SG  rec-b-ap-singapore-1.byteplusapi.com
  US  rec-us-east-1.byteplusapi.com
  US  rec-b-us-east-1.byteplusapi.com
  AIR byteair-api-cn1.snssdk.com
)

# request function
test(){
print_table(){
    printf "item\tavg\tP50\tP90\tP95\tP99\n"
    static "nlookup_cost" "$1"
    static "tcp_cost" "$2"
    static "ssl_cost" "$3"
    static "prepare_cost" "$4"
    static "redirect" "$5"
    static "start_receive" "$6"
    static "time_total" "$7"
    static "transfer" "$8"
}
...
time_total_transfer=()
    ...
    do
        status=`echo "$i % 4"|bc`
        printf "executing.... %c\r" "${running_array[$status]}"
        curlOut=`curl -m 5 -s -o /dev/null -w "$out_format" ${2}`
        ...
        do
            case $idx in
            1)
                time_namelookup[${#time_namelookup[@]}]=$var
                ;;
            2)
                time_connect[${#time_connect[@]}]=$var
                ;;
            3)
                time_appconnect[${#time_appconnect[@]}]=$var
                ;;
            4)
                time_pretransfer[${#time_pretransfer[@]}]=$var
                transfer=$var
                ;;
            5)
                time_redirect[${#time_redirect[@]}]=$var
                ;;
            6)
                time_starttransfer[${#time_starttransfer[@]}]=$var
                ;;
            7)
                time_total[${#time_total[@]}]=$var
                total=$var
                ;;
            esac
            idx=`expr $idx + 1`
        done
        time_total_transfer[${#time_total_transfer[@]}]=`echo "scale=2; $total-$transfer"|bc`
    done
    ...
    static(){
        ...
        len=${#sorted[*]}
        P50idx=`expr $len \* 50 / 100`
        P90idx=`expr $len \* 90 / 100`
        P95idx=`expr $len \* 95 / 100`
        P99idx=`expr $len \* 99 / 100`
        avg=`echo "scale=2; $avg*1000"|bc`
        p50=`echo "scale=2; ${sorted[$P50idx]}*1000"|bc`
        p90=`echo "scale=2; ${sorted[$P90idx]}*1000"|bc`
        p95=`echo "scale=2; ${sorted[$P95idx]}*1000"|bc`
        p99=`echo "scale=2; ${sorted[$P99idx]}*1000"|bc`
        printf "%s\t%-8.2f\t%-8.2f\t%-8.2f\t%-8.2f\t%-8.2f\n" $1 $avg $p50 $p90 $p95 $p99
    }
    # print
    ...
    printf "%-20s%s\n" request_count: ${1}
    # Print Table
    print_table "${time_namelookup[*]}" "${time_connect[*]}" "${time_appconnect[*]}" "${time_pretransfer[*]}" "${time_redirect[*]}" "${time_starttransfer[*]}" "${time_total[*]}" "${time_total_transfer[*]}" | format_table -1 -5
    printf "\n"
}
num=100
head=https://
...
if [ "$url"x != "x" ];then
    ...
    tail="/data/api/ping"
    hostsLen=${#hosts[*]}
    for((j=0;j<hostsLen;j+=2))
    do
      hostRegion=${hosts[j]}
      host=${hosts[j+1]}
      url=$head$host$tail
      test $num $url $hostRegion $host
    done
fi